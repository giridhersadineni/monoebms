...
if ($conn->connect_error) {
    ...
    $records_per_page = 10; // Change this to set the number of records per page
    ...
    $sql = "SELECT DISTINCT(HALLTICKET), EXAMID, EXAMNAME FROM RESULTS
            ...
            WHERE EXAMID > 250 ORDER BY HALLTICKET
                        ...
                        <div class="col-md-4">
                            <select class="form-control" id="examSelect">
                                ...
                                while ($examRow = $examResult->fetch_assoc()) {
                                    echo '<option value="' . $examRow['EXID'] . '">' . $examRow['EXAMNAME'] . '</option>';
                                }
                                ?>
                            </select>
                        </div>
...
<script>
    ...
    function fetchResults() {
        ...
        if (selectedExamId) {
            ...
            const url = `fetch_results.php?examid=${selectedExamId}`;
            fetch(url)
