   ...
   // echo $getcandidates;
    ...
    while($candidate=mysqli_fetch_assoc($candidates)){
     echo '<div class="resulttable">';
        ...
        $getnamequery='select sname,fname from students where haltckt ="'.$candidate['HALLTICKET'].'"';
        $getname=$conn->query($getnamequery);
        $name=mysqli_fetch_assoc($getname);
        //echo $getnamequery;
              echo"<b style='padding-left:1em;'>EID:</b> ".$candidate['EID']."</b>";
        echo "<b style='padding-left:1em;'>   Hallticket:  </b> ". $candidate['HALLTICKET']."<b>     Student Name  </b> " .$name['sname']."<b>    Father Name  </b>" .$name['fname'];
     
     echo '<br>';
    // echo '<p>'.$name['FULLNAME'].'</p>';
$sql="SELECT `PAPERCODE`,`INT`,`EXT`,`GRADE` FROM rholdernew WHERE HALLTICKET='".$candidate['HALLTICKET']."' AND EXAMID=".$_GET['exid']." AND GRADE !='#VALUE!' ORDER BY EXAMID";
  ...
  echo '<td style="padding:1em";><b>PCODE<br>INT<br>EXT<br>GRADE<b></td>';
