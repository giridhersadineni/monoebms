...
$deleteenrollments="DELETE FROM `PR_GPAS` where EXAMID=$examid and HALLTICKET=$hallticket";
echo $deletenrollments;
$deleted=$conn->query($deleteenrollments);
echo "Enrollement Deleted:";
var_dump($deleted);
    
       $sql="select * from RESULTS_STAGE where HALLTICKET=$hallticket and EXAMID=$examid";
       ...
       if($studentres->num_rows>0){
            echo "<hr>";
                ...
                $sgpa=round($sgpa,2);
                $cgpa=$sgpa;
                 $query="INSERT INTO `PR_GPAS`(`EXAMID`, `HALLTICKET`, `TOTAL`, `RESULT`, `SGPA`, `CGPA`, `PROCESSID`) values ('$examid','$hallticket',$total,'$result','$sgpa','$cgpa','singleupdate')";
                echo $query;
                if($conn->query($query)){
