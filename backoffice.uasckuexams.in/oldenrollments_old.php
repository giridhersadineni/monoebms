...
if ($conn->connect_error) {
    ...
    $sql = "select * from  examenrollments left join examsmaster on examenrollments.EXAMID=examsmaster.EXID where HALLTICKET='" . $_COOKIE['userid'] . "' order by SEMESTER";
...
//  echo "<center>";
         ...
         if(($row["STATUS"]=='NOTIFY' || $row["STATUS"]=='OPEN') && ($row["FEEPAID"] =='' ||$row["FEEPAID"] =='0' || $row["FEEPAID"] =='1'))
        {
          ...
          echo '<a href="printapplication.php?id=' . $row["ID"] . '" class="btn btn-info">Print Application</a>';
        }
