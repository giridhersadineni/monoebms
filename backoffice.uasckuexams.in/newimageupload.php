...
$student=get_one_assoc($getstudent);
if($student['aadhar']==""){
    $studentdata=implode(" ", $student);
    echo $studentdata;
    echo '"<script> 
    swal("Update Aadhar before Uploading Photo and Signature")
    .then((value) => {
...
if (isset($_POST['UPDATE'])) {
    ...
    if (file_exists("upload/images/" . $_COOKIE['aadhar'] . ".jpg")) {
        $photodeleted = unlink("upload/images/" . $_COOKIE['aadhar'] . ".jpg");
    }
    if (file_exists("upload/images/" . $_COOKIE['aadhar'] . ".jpg")) {
        $signdeleted = unlink("upload/signatures/" . $_COOKIE['aadhar'] . ".jpg");
    }
...
?>
 ...
 <form action="newimageupload.php" method="POST" onsubmit="return validate()" id="uploadform" enctype="multipart/form-data">
  <h3>Upload Image:<span>(50kb)</span></h3>
  <input type="file" class="form-control " name="photo" id="studimage"><br><br>
  <h3>Upload Signature:<span>(30kb)</span></h3>
...
?>
<center>
  <input type="submit" name="UPDATE" value="Update Photo And Signature" class="btn btn-primary" >
</center>
</form>
