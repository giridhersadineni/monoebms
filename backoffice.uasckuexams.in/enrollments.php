...
if ($conn->connect_error) {
    ...
    $sql = "select * from  examenrollments left join examsmaster
    on examenrollments.EXAMID=examsmaster.EXID where HALLTICKET='" . $_COOKIE['userid'] . "' order by SEMESTER,EXID DESC";
...
//  echo "<center>";
        if( $row['STATUS']=="CLOSED" || $row['STATUS']=="HIDDEN" ){
           ...
           if($row['STATUS']=="CLOSED" && $row['EXAMID'] > 298 ){
        ...
        if(isset($_GET['testmode'])){
               echo '<p style="text-align:right;"><input type="submit" class="btn btn-danger" value="Get Result (Test Mode)"></input>';
        }
