...
<head>
    ...
    <script>
        ...
        ga('set', 'userId', '<?php echo htmlspecialchars($_COOKIE['name'], ENT_QUOTES, 'UTF-8'); ?>'); // Set the user ID using signed-in user_id.
    </script>
    <style>
        table {
            border-style: solid;
...
<body>
    ...
    // Using prepared statement for the second query
    ...
    <table align='center'>
        ...
        if ($result->num_rows > 0) {
            while ($paper = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td style='text-align:center;'>" . htmlspecialchars($paper['PAPERCODE'], ENT_QUOTES, 'UTF-8') . "</td>";
                echo "<td>" . strtoupper(htmlspecialchars($paper['PAPERNAME'], ENT_QUOTES, 'UTF-8')) . "</td>";
                echo "<td style='text-align:center;'>" . htmlspecialchars($paper['CREDITS'], ENT_QUOTES, 'UTF-8') . "</td>";
